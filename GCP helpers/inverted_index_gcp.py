    ...
    def read(self, locs, n_bytes):
        ...
        for f_name, offset in locs:
            if f_name not in self._open_files:
                blob = self.bucket.blob(f"{self._base_dir}/{f_name}")
...
class InvertedIndex:
    ...
    def posting_lists_iter(self):
        ...
        """
        with closing(MultiFileReader(self.bucket, self.base_dir)) as reader:
    ...
    def read_posting_list(self, w):
        with closing(MultiFileReader(self.bucket_name, self.bin_dir)) as reader:
    ...
    def read_index(project, bucket_name, name, bin_dir='.'):
        ...
        if isinstance(index, InvertedIndex):
            ...
            index.bin_dir = bin_dir
        return index
    
    @staticmethod
    def read_csv(project, bucket_name, name):
        fs = gcsfs.GCSFileSystem(project=project)
        with fs.open(f'{bucket_name}/{name}.csv.gz') as f:
            df = pd.read_csv(f, compression='gzip', delimiter=',', index_col=0, header=None)
        index = df.squeeze('columns').to_dict()
        return index
    ...
    @staticmethod
    def write_a_posting_list(folder, b_w_pl, bucket_name):
